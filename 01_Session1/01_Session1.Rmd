...
Séance 1. L'information géographique : données, structures et outils

  - *Données spatiales avec R : format, manipulation*

<br><br>

Séance 2. Cartographie : sémiologie graphique

  - *Construire une carte avec R*
  
<br><br>

Séance 3. Données spatiales et statistiques
...
```
.center[.tiny[Boy & Uitermark (2017) Reassembling the city through Instagram. *Transactions of the
...
- Données textuelles le plus précis possible (associer rue, code postal etc) dans un vecteur.
...
a <- geocode_OSM(mydf$Adresse, output = "more", source = "google")
```
...
4. Créer un objet SF qui géolocalise les entreprises du CAC40.
...
.right-plot[
```{r plot-label1-out, ref.label="plot-label1", echo=FALSE, message = F, warning=F}
```
]
